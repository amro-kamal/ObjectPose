...
class Model(nn.Module):
    def __init__(self, device,model_name='inception_v3'):
        super(Model, self).__init__()
        self.device = device
        self.model_name=model_name
        self.net = models.__dict__[model_name](pretrained=True)
        self.net.eval()
...
def class_act_map(model,image_path, show_images=False):
...
# def run_clip(model, dataloader,imagenet_classes,correct_class,savepath, pose):
#     '''
#     Function to run clip-ViT-B/32 model
#     params: 
#         dataloader: bs = len(data)
#         imagenet_classes: python list
#         correct_class: true label
#         savepath: path to save the clip_log.txt file and clip_result.txt file
#     return:
...
#     result_file = open (os.path.join(savepath,model_name+"_result.txt"), "w")
...
#     for i in range(values.shape[0]): #For the 1000 images
#       if correct_class in indices[i]: #indices[i].shape = [5]
#         correct5+=1
#       if correct_class==indices[i][0]:
#         correct+=1
...
#         correct_f_conf.append(similarity[i][correct_class].item())


#     num_images=360
#     print(f'top1 {correct/num_images}, top5 {correct5/num_images}')
#     correct_t_conf = np.mean(correct_t_conf)
#     wrong_conf = np.mean(wrong_conf)
#     correct_f_conf = np.mean(correct_f_conf)
#     data =[[pose, num_images, correct, correct5, correct_t_conf, correct_f_conf, wrong_conf, 'clip-ViT-B/32']] 
...
#     result_file.close()
#     print (table)
#     return correct, correct5, np.mean(correct_t_conf), np.mean(correct_f_conf), np.mean(wrong_conf), result
